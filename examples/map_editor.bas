...
BTN_NEXT_X0 = 162 : BTN_NEXT_Y0 = 4 : BTN_NEXT_X1 = 184 : BTN_NEXT_Y1 = 24
...
DO
  ...
  IF L = 1 AND PREV_L = 0 THEN
    ...
    IF MX >= BTN_PREV_X0 AND MX <= BTN_PREV_X1 AND MY >= BTN_PREV_Y0 AND MY <= BTN_PREV_Y1 THEN
      CycleMap(-1)
    END IF
    IF MX >= BTN_NEXT_X0 AND MX <= BTN_NEXT_X1 AND MY >= BTN_NEXT_Y0 AND MY <= BTN_NEXT_Y1 THEN
      CycleMap(1)
    END IF
...
FUNCTION DrawChrome()
  ...
  FILLRECT BTN_NEXT_X0, BTN_NEXT_Y0 TO BTN_NEXT_X1, BTN_NEXT_Y1
  ...
  RECT BTN_NEXT_X0, BTN_NEXT_Y0 TO BTN_NEXT_X1, BTN_NEXT_Y1
  ...
  DRAWTEXT BTN_NEXT_X0 + 8, BTN_NEXT_Y0 + 8, ">"
...
FUNCTION DrawPicker()
  ...
  COLORRGB 80, 80, 100
  FOR PI = 0 TO PICK_COUNT - 1
    GX = PICKER_X + (PI MOD PCOLS) * TILE_SIZE
    GY = PICKER_Y + (PI \ PCOLS) * TILE_SIZE
  ...
  FOR VR = 0 TO VIEW_ROWS - 1
    FOR VC = 0 TO VIEW_COLS - 1
      ...
      IF WC >= 0 AND WC < MCOLS AND WR >= 0 AND WR < MROWS THEN
        VAL = MAP(WR * MCOLS + WC)
      END IF
      VIEW_BUF(VR * VIEW_COLS + VC) = VAL
    NEXT VC
  NEXT VR
  TILEMAP DRAW 0, MAP_X, MAP_Y, VIEW_COLS, VIEW_ROWS, VIEW_BUF()
  COLORRGB 80, 80, 100
  FOR PI = 0 TO VIEW_COLS * VIEW_ROWS - 1
    GX = MAP_X + (PI MOD VIEW_COLS) * TILE_SIZE
    GY = MAP_Y + (PI \ VIEW_COLS) * TILE_SIZE
    RECT GX, GY TO GX + TILE_SIZE - 1, GY + TILE_SIZE - 1
  NEXT PI
END FUNCTION
