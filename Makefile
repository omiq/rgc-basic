...
basic-wasm-canvas:
	...
	$(EMCC) -w -O2 -s WASM=1 -DGFX_VIDEO -Igfx \
		-s EXPORTED_FUNCTIONS='["_basic_apply_arg_string","_basic_load_and_run_gfx","_wasm_push_key","_wasm_gfx_rgba_ptr","_wasm_gfx_rgba_version_read","_wasm_canvas_build_stamp","_wasm_canvas_set_debug","_wasm_canvas_set_debug_trace_for","_wasm_debug_log_stacks"]' \
		-s EXPORTED_RUNTIME_METHODS='["ccall","cwrap","FS","HEAPU8","wasmMemory","getValue"]' \
		-s FORCE_FILESYSTEM=1 -s NO_EXIT_RUNTIME=1 \
