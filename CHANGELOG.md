...
- **Gamepad (basic-gfx native):** **`JOY(port, button)`**, alias **`JOYSTICK`**, and **`JOYAXIS(port, axis)`** — Raylib polling in the main thread (`gfx_gamepad.c`). Buttons use Raylib enum values **1–15**; axes **0–5** return scaled **-1000..1000**. Canvas WASM returns **0** (stubs). Example **`examples/gfx_joy_demo.bas`**.

- **Tilemap sprite sheets:** **`LOADSPRITE slot, "path.png", tw, th`** defines a **tw×th** tile grid; **`SPRITETILES(slot)`**, **`DRAWSPRITETILE slot, x, y, tile_index [, z]`** (1-based tile index), **`gfx_sprite_tile_source_rect`**, **`SPRITEW`/`SPRITEH`** return one tile size when tilemap is active. **`SPRITECOLLIDE`** uses tile size when no explicit crop.
