...
**Status:** **`HTTPFETCH`**, **`OPEN` binary prefixes** (`rb:`/`wb:`/`ab:`), **`PUTBYTE`**, and **`GETBYTE`** are implemented in the interpreter (see **`CHANGELOG.md`**). Remaining items below are optional follow-ups.
...
- **`docs/ide-wasm-tools.md`** — **`basic_load_and_run_gfx`**, **`ARG$`** (CLI), MEMFS.
