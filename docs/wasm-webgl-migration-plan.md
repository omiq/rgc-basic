...
### 4.4 Deployment and integration script touchpoints

Three scripts must be updated when the raylib-emscripten WASM bundle lands. They are shared across repos, so the migration lands cleanly only if all three change in lock-step.

**`rgc-basic/scripts/pullmake.sh`** — central build orchestrator (builds native + all WASM variants; copies `web/*.*` to `8bitworkshop/rgc-basic`; runs WP asset copy):

- Add `make basic-wasm-raylib` after `make basic-wasm-canvas` (around line 48).
- `cp web/*.*` glob already picks up `basic-raylib.js` + `.wasm` — no change needed there.
- Ensure new `web/raylib.html` shell is present so the `cp web/*.*` copies it.

**`rgc-basic/scripts/verify-canvas-wasm.sh`** — gate that the canvas build hasn't regressed:

- Add a sibling `verify-raylib-wasm.sh` that boots the raylib iframe in Playwright and asserts a non-blank framebuffer from a canonical `.bas`.
- `pullmake.sh` should call both before the copy step.

**`8bitworkshop/scripts/sync-rgc-basic.sh`** — IDE-side submodule pull + validated build + copy to `gen/`:

- Build list (line 122): add `basic-wasm-raylib`.
- `REQUIRED_EXPORTS` (lines 132-136): the raylib path uses GLFW callbacks rather than the current `_wasm_gfx_key_state_ptr` / `_wasm_gfx_key_state_clear` / `_wasm_push_key` trio. Define the new set of required exports during raylib-path implementation (TBD — likely at least: the interpreter entry point, the key-state shim, the Asyncify sleep deadline symbol). Grep against `web/basic-raylib.js`.
- Copy step (lines 162-166): add `cp -v web/basic-raylib.{js,wasm} "$IDE_ROOT/gen/"` and, to fulfil the script's own header comment, also copy into `"$IDE_ROOT/rgc-basic/"` for the canvas + raylib + modular bundles (see pre-existing gap noted below).

**`8bitworkshop/deploy.sh`** — rsync staging + server upload:

- Cache-buster comment at line ~50 currently mentions only `basic-canvas.js and .wasm`. Update to cover both bundles.
- Verification block around lines 360-375: add `test -f $STAGING_DIR/rgc-basic/basic-raylib.js` and `.wasm`.
- Post-deploy readable-files check around line 731: add `test -r $SERVER_PATH/rgc-basic/basic-raylib.js` and `.wasm`.
- Public repo mirror section (if `PUBLISH_PUBLIC_REPO=1`, lines 800-930): the `cp -r rgc-basic` at line ~863 already copies everything in the dir. No new code needed beyond updating the verification block.

**`rgc-basic-iframe.html`** (lives in `8bitworkshop/rgc-basic-iframe.html`, cache-buster-substituted by deploy.sh):

- Read `?renderer=` query param. Default `canvas` → `canvas.html`. `raylib` → `raylib.html`. Any other value → `canvas.html` with a console warning.

**`rgc-basic/wordpress/rgc-basic-tutorial-block/copy-web-assets.sh`** and the WP plugin release pipeline:

- Audit to confirm it copies whatever pattern it uses (`*.wasm`, `*.js`) rather than hardcoded filenames. If it hardcodes, extend to include the raylib pair.
- The plugin must ship both bundles until canvas retirement (§4.5).

**`src/platform/rgc-basic-asset-cb.gen.ts`** header comment (auto-generated by `deploy.sh`):

- Update template in `deploy.sh:48-53` to say "for `basic-canvas.*` and `basic-raylib.*`".

### 4.5 Pre-existing script inconsistencies (fix separately)

These are unrelated to the migration but are worth fixing to reduce confusion during the parallel period. Not blockers; tracked here so they don't get forgotten.

1. `scripts/sync-rgc-basic.sh:22` comment says artifacts are copied "to rgc-basic/ (where the iframe loads them) and gen/" but the code (lines 162-166) only copies to `gen/`. The `rgc-basic/` dir that `deploy.sh:731` reads is populated by `pullmake.sh`, not by `sync-rgc-basic.sh`. Fix: either update the comment to match code, or actually copy to `$IDE_ROOT/rgc-basic/` as the comment claims.
2. `sync-rgc-basic.sh:122` builds `basic-wasm-canvas basic-wasm-modular` but not `basic-wasm` (the terminal WASM build). Line 166 then tries `cp web/basic.{js,wasm}` with `|| true`, silently skipping if stale from an earlier build. Fix: either add `basic-wasm` to the build line, or remove the copy (if terminal WASM isn't actually needed by the IDE).
3. `sync-rgc-basic.sh` does not run `wordpress/rgc-basic-tutorial-block/copy-web-assets.sh` — only `pullmake.sh` does. If a collaborator uses `sync-rgc-basic.sh` alone and publishes a WP plugin release, it will ship stale assets. Fix: either add the WP copy step, or document that `sync-rgc-basic.sh` is IDE-only and `pullmake.sh` is required before a WP release.
