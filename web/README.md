...
**Embedding in another site (e.g. an online IDE)** — see **[`docs/ide-retrogamecoders-canvas-integration.md`](../docs/ide-retrogamecoders-canvas-integration.md)** for hosting `basic-canvas.js` / `.wasm`, MEMFS assets, Run/Pause/Stop, and iframe `postMessage` patterns. **IDE-hosted tools** (PNG preview, etc.) — pass **`ARG$(1)`** via **`basic_load_and_run_gfx_argline`**: **[`docs/ide-wasm-tools.md`](../docs/ide-wasm-tools.md)**.
...
The terminal build (`basic.js`) enables **Asyncify** (`emscripten_sleep` and async **`HTTP$`** / **`HTTPFETCH`**) so **INPUT**, **GET**, and **INKEY$** can block without freezing the tab; call `basic_load_and_run` with **`ccall(..., { async: true })`** so the returned Promise resolves when the program finishes. Before each **`HTTP$`/`HTTPFETCH`** fetch, the runtime flushes buffered **`PRINT`** output (stdout or canvas RGBA) so text above the call is visible while the request runs. **`LOADSPRITE`** paths must be **MEMFS/local** — not **`https://`** URLs; use **`HTTPFETCH`** first or **`FS.writeFile`** from JS — see **`docs/wasm-assets-loadsprite-http.md`**. **`index.html`** exposes **Pause** / **Resume** (`Module.wasmPaused`) and **Stop** (`Module.wasmStopRequested`), same flags as the canvas demo. **`Module.wasmRunDone`** is set when `basic_load_and_run` finishes. **`HTTP$`/`HTTPSTATUS`** and **`HTTPFETCH`** use **`fetch`**; remote APIs must send **CORS** headers your origin is allowed to read. **`HTTPFETCH(url$, path$)`** writes the raw response body to a MEMFS path (binary-safe); then **`OPEN 1,1,0,"rb:…"`** + **`GETBYTE`** / **`LOADSPRITE`** as needed.

The **canvas** build (`basic-canvas.js`) links **`GFX_VIDEO`** with **`gfx_canvas.c`** (PETSCII + bitmap) and **`gfx_software_sprites.c`** (PNG **`LOADSPRITE`** / **`DRAWSPRITE`** via stb_image). Use **`basic_load_and_run_gfx`**; for **trailing args** ( **`ARG$(1)`** … ) use **`basic_load_and_run_gfx_argline`** — see **`docs/ide-wasm-tools.md`**. The page’s rAF loop reads **`wasm_gfx_rgba_ptr()`** / **`wasm_gfx_rgba_version_read()`** (see **`canvas.html`**). **`#OPTION columns 80`** selects a **640×200** framebuffer (else 320×200). **`SCREEN 1`** draws the **320×200** hires bitmap like **basic-gfx**. **`INPUT`** and **`GET`/`INKEY$`** use the canvas; **`#OPTION border`** pads the framebuffer in the browser (same idea as Raylib). Put PNGs on **`Module.FS`** (e.g. upload or `writeFile`) using paths your program expects — **`LOADSPRITE`** paths are **VFS/local**, not raw **http://** URLs (use **`HTTPFETCH`** to download into MEMFS, or **`HTTP$`** for small text). Details: **`docs/gfx-canvas-parity.md`**.
...
- **`PEEK(56320 + code)`** (same as **`GFX_KEY_BASE`** / C64-style keyboard matrix): canvas updates **`key_state[]`** from keyboard events (via **`HEAPU8`** at **`wasm_gfx_key_state_ptr`**, so polling works even during **`SLEEP`** / Asyncify). Demos like **`examples/gfx_key_demo.bas`** / **`gfx_jiffy_game_demo.bas`** match **basic-gfx** when the canvas is focused; keys are cleared when you click **Run**.
