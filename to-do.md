...
* **Bitmap graphics & sprites** (incremental; see `docs/bitmap-graphics-plan.md`, `docs/sprite-features-plan.md`)
  ...
  2. **Sprites (minimal)** — ~`LOADSPRITE` / `DRAWSPRITE` (with z + optional `sx,sy,sw,sh` crop) / `SPRITEVISIBLE` / `SPRITEW`/`SPRITEH`~ in basic-gfx; PNG alpha over text/bitmap. Still open: `SPRITECOLLIDE`, tilemap `LOADSPRITE` mode.
