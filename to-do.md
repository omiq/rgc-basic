...
| ~**12**~ | ~Sound MVP~ | **Done (1.11.0, 2026-04-19).** Ships `LOADSOUND / PLAYSOUND / STOPSOUND / UNLOADSOUND / SOUNDPLAYING()` in native basic-gfx + basic-wasm-raylib via `gfx/gfx_sound.{c,h}`. Single voice, WAV only. `PAUSESOUND`/`RESUMESOUND`/`SOUNDVOL`/loop/second-voice deferred. Canvas WASM stays frozen — runtime error if used there. Example `examples/gfx_sound_demo.bas` + bundled `blip.wav`/`boom.wav`. |
...
* **Trek tutorial refactor roadmap (2026-06-02).** North star: galaxy matrix, factions/fleets, non-combat tactics, eventual multiplayer. Phased plan and debt list in **`docs/trek-refactor-next-steps.md`**.

* **UDF inside `PRINT` — trek-new still uses assign-then-print (2026-06-02).** Shipped fix: expression calls pass `caller_resume_pos` (parse cursor after the closing `)`) into `invoke_udf` and restore it on return, so a following bare `PRINT` on the next line emits its newline again (`tests/udf_print_inline_next.bas`; CHANGELOG 2026-06-02). `examples/trek-new.bas` still keeps `QN$=QuadrantName$(…)` then `PRINT "IN THE QUADRANT, ";QN$;"."` (and the same for `ENTERING …`) because the full deterministic regression still drops the blank line before `Pause()` when `QuadrantName$` is inlined in the `PRINT` list, even though smaller repros pass. Follow-up: reproduce with full `trek-new.bas` load + `Pause()` / PETSCII / `EnterQuadrant` context; confirm whether another cursor or `print_col` issue remains; then drop the `QN$` workaround and refresh goldens if inline form is safe.
...
- **`ON expr GOTO/GOSUB` only accepts numeric line numbers, not labels** (`statement_on`, `basic.c:15743`). A fully label-based program (no line numbers) therefore can't use computed dispatch and has to expand each `ON` into a stack of `IF expr = n THEN GOTO label` lines (see `examples/trek-new.bas`, converted 2026-06-01). Enhancement: teach `statement_on` to resolve a comma list of labels as well as line numbers, so label-only sources keep the compact dispatch form. **Largely superseded 2026-06-01** by `SELECT CASE` (below), which is the preferred modern dispatch.
- **[ADDED 2026-06-01] `SELECT CASE … CASE … CASE ELSE … END SELECT`.** Block multi-way dispatch (numeric/string selector; comma lists; `CASE IS <relop> expr`; `CASE lo TO hi`; `CASE ELSE`; nestable to depth 16). Same block-stack discipline as block `IF`/`DO`/`WHILE`: per-frame selector, snapshotted across `FUNCTION` calls (`udf_call_frame.saved_select_top`), unwound to the running UDF's floor on `GOTO` so a `GOTO` out of a `CASE` body inside a function is safe. Implemented in `basic.c` (`statement_select`/`statement_case`/`statement_end_select`, scanners `select_scan_to_match`/`select_skip_to_end`). Tests: `tests/select_case_test.bas`; demo `examples/select_case.bas`. Lint tier `modern` (`SELECT`,`CASE` in `rules.json`/`spec.json`). README + CHANGELOG + retrodocs `language.md` updated same change-set. Used throughout `examples/trek-new.bas`: the main state loop, the command dispatch, `DeviceName`, `QuadrantName`, the Computer function dispatch, the Roman-numeral and SRS side-panel selectors, and the galaxy-generation threshold cascade are now `SELECT CASE` (all `GOSUB`s gone, all *cross-routine* `GOTO`s gone; remaining `GOTO`s are intra-routine combat/movement/render/input state machines where labelled jumps are idiomatic and lack deterministic test coverage to refactor safely). Followed up 2026-06-01 by `EXIT FOR` + `CONTINUE` (next entry), which dissolved 14 of those combat/render/movement `GOTO`s.
- **[ADDED 2026-06-01] `EXIT [DO|FOR|WHILE]` + `CONTINUE FOR|DO|WHILE`.** Structured loop break / skip-to-next-iteration, so a loop body no longer needs `GOTO` to a label sitting on the closing `NEXT`/`LOOP`/`WEND`. Bare `EXIT` and `EXIT DO` keep their historic meaning (innermost `DO`); `EXIT FOR` leaves the innermost `FOR`/`FOREACH`, `EXIT WHILE` the innermost `WHILE`. `CONTINUE` requires a kind keyword (no ambiguous bare form) and lands execution on the loop's closer so its normal increment/test runs. **Footgun closed:** each `for`/`do`/`while` frame now records `if_depth`/`select_top` at entry and `EXIT`/`CONTINUE` restore to it, so jumping out of a block `IF` or `SELECT CASE` in the body no longer orphans those frames (also fixes the latent case for the old bare `EXIT`). Implemented in `basic.c` (`statement_exit`/`statement_continue`, land-on scanners `continue_to_next`/`continue_to_loop`/`continue_to_wend`; EXIT reuses the existing `skip_*` scanners). Tests: `tests/exit_continue_test.bas`; demo `examples/exit_continue.bas`. Lint: `EXIT` portable, `CONTINUE` tier `modern`. README + CHANGELOG + retrodocs `language.md` updated same change-set. Applied to `examples/trek-new.bas`: phasers, Klingon return-fire, Klingon movement, device repair, warp movement, the SRS dock-check and the galaxy-record header/cell loops are now `CONTINUE FOR` (14 conversions; cross-routine `GOTO` total 97→83, all verified byte-exact across 3 deterministic-seed scenarios incl. a combat run). Remaining `GOTO`s are genuine non-local breaks (e.g. `GOTO CrossQuadrant`) or uncovered torpedo loops (the find-loop's `I=3` fallback isn't a clean `EXIT FOR`).
