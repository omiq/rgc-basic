    ...
    try:
        with sync_playwright() as p:
            ...
            page.wait_for_function(
                "() => document.querySelectorAll('.rgc-tutorial-embed').length === 2",
                timeout=120000,
            )
            page.wait_for_function(
                "() => document.querySelectorAll('.rgc-tutorial-toolbar button').length >= 2",
                timeout=120000,
            )
            # First embed: wait for Run enabled, click first Run only
            page.wait_for_function(
                """() => {
                  const b = document.querySelector('#ex1 .rgc-tutorial-toolbar button');
                  return b && !b.disabled && b.textContent === 'Run';
                }""",
                timeout=120000,
            )
            page.evaluate("document.querySelector('#ex1 .rgc-tutorial-toolbar button').click()")
            page.wait_for_function(
                "() => (document.querySelector('#ex1 .rgc-tutorial-output').textContent || '').includes('FIRST EXAMPLE')",
                timeout=120000,
            )
            # Second embed (no editor): second Run button
            page.wait_for_function(
                """() => {
                  const rows = document.querySelectorAll('#ex2 .rgc-tutorial-toolbar button');
        ...
        # Full getting-started page: ten independent embeds
        with sync_playwright() as p:
            browser = p.chromium.launch(headless=True)
            page = browser.new_page(viewport={"width": 1100, "height": 900})
            page.goto(
            ...
            page.wait_for_function(
                "() => document.querySelectorAll('.rgc-tutorial-embed').length === 10",
                timeout=120000,
            )
            page.wait_for_function(
                """() => {
