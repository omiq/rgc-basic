...
- **Blitter surfaces (`IMAGE NEW` / `IMAGE FREE` / `IMAGE COPY` / `IMAGE SAVE`):** AMOS/STOS-style 1bpp off-screen bitmaps for scroll/parallax/work-buffer patterns. Slot `0` is the live visible bitmap; slots 1..31 are caller-allocated. `IMAGE COPY src, sx, sy, sw, sh TO dst, dx, dy` clips both rects and handles overlap. `IMAGE SAVE slot, "path.bmp"` exports as 24-bit BMP (no external dep). Scrolling: `examples/gfx_scroll_demo.bas`. Parallax: `examples/gfx_parallax_demo.bas`.
