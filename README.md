...
Current release: **1.10.0** (2026-04-19) — non-gfx utility batch: `TICKUS()/TICKMS()` monotonic timers, `CWD$()/CHDIR`, `DIR$(path)/DIR path INTO arr$,count` directory listing, `JSONLEN/JSONKEY$` iteration, and `FOREACH var IN arr ... NEXT`. On top of 1.9.9's pixel-perfect `ISMOUSEOVERSPRITE(slot, alpha_cutoff)`, 1.9.8's `SPRITEAT(x, y)` + SCROLL-aware `ISMOUSEOVERSPRITE`, 1.9.7's `CLS rect` + `DRAWTEXT scale`, 1.9.6's AMOS-style multi-plane screen buffers (`SCREEN BUFFER/DRAW/SHOW/FREE/SWAP/COPY`, 8 slots), 1.9.5's bitmap-plane double-buffer, and the `FILEEXISTS` / `DOWNLOAD` / `IMAGE GRAB/SAVE` / compound-assignment (`+= -= *= /= ++ --`) work from 1.9.2–1.9.4. Graphics 1.0 milestone (1.9.0): full 2-D primitive set, blitter Phase 1, tilemap renderer, VSYNC pipeline, keyboard intrinsics, anti-alias toggle, two-word SPRITE/TILE/SHEET/IMAGE family. Next: **2.0** with sound. See `CHANGELOG.md` for the full versioned history and `docs/release-graphics-1.0.html` for the announcement page.
