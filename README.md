...
Current release: **1.9.1** (2026-04-18) — polish pass on top of the **Graphics 1.0** milestone (1.9.0): per-cell `PAPER` background colour, `KEY_UP/DOWN/LEFT/RIGHT/SPACE/ENTER/ESC/TAB/BACK` named key-code constants, default lower-ROM charset so ASCII upper+lower both render natively, `#OPTION PETSCII` now respects an explicit `#OPTION CHARSET`, `FILLRECT` with `COLOR 0` clears pixels in bitmap mode, Unicode box-drawing (`┌─┐│└┘`) renders in the default non-PETSCII mode, plus WASM raylib fixes (forced end-of-run refresh, 60 Hz tick advance for `ANIMFRAME`, window resize on `#OPTION COLUMNS 80`, runtime errors forwarded to the IDE, local cache-bust script). Graphics 1.0 milestone itself (1.9.0): full 2-D primitive set, blitter Phase 1, tilemap renderer, VSYNC pipeline, keyboard intrinsics, anti-alias toggle, two-word SPRITE/TILE/SHEET/IMAGE family. Next: **2.0** with sound. See `CHANGELOG.md` for the full versioned history and `docs/release-graphics-1.0.html` for the announcement page.
...
- **Blitter surfaces (`IMAGE NEW` / `IMAGE FREE` / `IMAGE COPY` / `IMAGE GRAB` / `IMAGE LOAD` / `IMAGE SAVE`):** AMOS/STOS-style off-screen bitmaps for scroll/parallax/work-buffer patterns. Slot `0` is the live visible bitmap; slots 1..31 are caller-allocated. `IMAGE COPY src, sx, sy, sw, sh TO dst, dx, dy` clips both rects and handles overlap. `IMAGE GRAB slot, sx, sy, sw, sh` snapshots a region of the currently-displayed framebuffer: on basic-gfx and canvas WASM this captures the fully-composited RGBA frame (bitmap + text + sprites + tilemap cells, full palette and alpha); on builds without a full-frame hook it falls back to a 1bpp copy of the bitmap plane. `IMAGE SAVE slot, "path"` auto-routes on extension: `.png` writes 32-bit RGBA (preserves alpha from a grab, or treats slots 1..31 as a transparent-off mask when they're still 1bpp); anything else writes 24-bit BMP. Scrolling: `examples/gfx_scroll_demo.bas`. Parallax: `examples/gfx_parallax_demo.bas`. Screenshot / frame export for ffmpeg: `examples/gfx_screenshot_demo.bas`.
...
- **Mouse-over-sprite hit testing (tutorial pattern):** combine `GETMOUSEX/Y` with `SPRITEW/H` to turn any sprite into a clickable button or draggable object — useful for RTS unit select, RPG inventory, or menus. A dedicated `ISMOUSEOVERSPRITE(slot)` engine function is planned (see `docs/mouse-over-sprite-plan.md`); until it ships, the BASIC helper below covers the bounding-box case:
