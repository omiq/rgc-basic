...
DRAW 0` restores). `SCROLL ZONE id, y, h` + `SCROLL ZONE id, dx`
declare up to 15 horizontal bands that scroll independently at
composite time (with free wrap); `SCROLL LINE y, dx` sets
per-scanline horizontal offset for raster warp / water ripple /
flag-wave effects without raster interrupts. `SCROLL RESET` nukes
all of it. `DRAWTEXT` now honours inline PETSCII tokens: 16 colour
bytes (`{RED}`, `{WHITE}`, `{GREEN}`, …) swap the pen mid-string
so one call paints multi-colour text; `{REVERSE ON}` / `{REVERSE
OFF}` toggle reverse-video with two flavours — solid bg (classic
...
- `-v` / `-V` / `--version`: print the build's version tag, release date, and variant (`basic` / `basic-gfx` / `basic-wasm-raylib`) then exit. Version string comes from `git describe --tags --dirty --always` at build time, so release binaries report the exact tag they were cut from (e.g. `v2.0.0`), dev builds report `v2.0.0-3-gABCDEF-dirty` style. Useful for bug reports and checking you're on the latest.
...
- **`IMAGE DRAW slot`** *(basic-gfx / basic-wasm-raylib, 2.1)*: retarget every SCREEN 2 / SCREEN 4 RGBA primitive into an off-screen `IMAGE CREATE` surface instead of the live framebuffer. Same shape as `SCREEN DRAW n` but routed into the IMAGE pool so the canvas size is arbitrary (not bound to the 320×200 / 640×400 screen). `IMAGE DRAW 0` restores. Use it to pre-bake wide scroller strips, world maps, HUD layers, reusable sprite atlases — then blit back with `IMAGE BLEND`. Non-RGBA screen modes return a runtime error. Example: `examples/gfx_imagedraw_demo.bas` pre-bakes a 1024×64 gradient-into-glyphs strip then scrolls a window of it each frame.
- **`SCROLL ZONE id, y, h` / `SCROLL ZONE id, dx` / `SCROLL ZONE CLEAR id` / `SCROLL ZONE RESET id`** *(basic-gfx / basic-wasm-raylib, 2.1)*: declare up to 15 horizontal bands (ids 1..15) on the RGBA screen that scroll independently at composite time. `id, y, h` declares the rect; `id, dx` advances the running horizontal offset (wraps modular on plane width, state persists across frames so one line per frame = a smooth scroll). Classic demo "message bar" / foreground-only scroll / three-layer parallax.
- **`SCROLL LINE y, dx` / `SCROLL LINE RESET`** *(basic-gfx / basic-wasm-raylib, 2.1)*: per-scanline horizontal offset for raster warp effects (water ripple, flag wave, heat haze, CRT jitter). Each row gets its own `dx`; layer on top of a `SCROLL ZONE` for "scrolling text with ripple". `SCROLL RESET` wipes both zones and per-line state. Example: `examples/gfx_scrollzone_demo.bas` shows three simultaneous layers (top zone, middle sine ripple, bottom zone scrolling opposite direction).
- **Inline PETSCII tokens in `DRAWTEXT`** *(basic-gfx / basic-wasm-raylib, 2.1)*: one `DRAWTEXT` call can paint multi-colour + reverse-video text. 16 C64 colour tokens (`{BLACK}` `{WHITE}` `{RED}` `{CYAN}` `{PURPLE}` `{GREEN}` `{BLUE}` `{YELLOW}` `{ORANGE}` `{BROWN}` `{PINK}` `{DARKGRAY}` `{MEDGRAY}` `{LIGHTGREEN}` `{LIGHTBLUE}` `{LIGHTGRAY}`) swap the pen mid-string; `{REVERSE ON}` / `{REVERSE OFF}` toggle reverse-video. Extended form `DRAWTEXT x, y, text$, fg, bg [, font [, scale]]`: `bg >= 0` = solid cell (classic Commodore swap, glyph shows bg colour), `bg < 0` = transparent glyph (cell fills with fg, glyph pixels stay untouched so whatever is painted underneath — a gradient, a photo — reads through the letter shape for a "gradient-coloured text" / knock-out effect). Cursor-move tokens (`\n` / `\r` / `{HOME}` / `{CLEAR}` / `{DOWN}` / `{UP}` / `{LEFT}` / `{RIGHT}`) are consumed silently — DRAWTEXT is pixel-space, use separate calls for multi-line layout. Example: `examples/gfx_drawtext_tokens_demo.bas`.
...
- **Music / tracker modules (basic-gfx + basic-wasm-raylib):** **`LOADMUSIC slot, "song.mod"`** streams a tracker module (**MOD** / **XM** / **S3M** / **IT**) or a long-form **OGG** / **MP3** via raylib's `raudio` (slots 0..7, separate pool from `LOADSOUND`). **`PLAYMUSIC slot`** / **`STOPMUSIC slot`** / **`PAUSEMUSIC slot`** / **`RESUMEMUSIC slot`** drive playback. **`MUSICVOLUME slot, 0.0..1.0`** sets per-slot gain, **`MUSICLOOP slot, 0|1`** toggles repeat-on-end, **`UNLOADMUSIC slot`** frees the stream. Functions: **`MUSICPLAYING(slot)`** is **1** while audible; **`MUSICLENGTH(slot)`** returns total track length in seconds (MOD files are computed via a pattern/tempo walk at load time — jar_mod's built-in length probe freezes Windows MinGW for tens of seconds, so we avoid it); **`MUSICTIME(slot)`** returns elapsed seconds (wraps on loop); **`MUSICPEAK()`** is the **0..1** master-mix held-peak, suitable for VU bars. Autoplay policy still applies on the browser build — wait for a keypress / click before the first `LOADMUSIC`. Example: `examples/gfx_music_demo.bas` with bundled Public-Domain tracks in `examples/music/` (see `LICENSES.md`).
