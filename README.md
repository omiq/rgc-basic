...
Current release: **1.9.8** (2026-04-19) — `SPRITEAT(x, y)` topmost-hit lookup (Z tie-break) + `ISMOUSEOVERSPRITE` now SCROLL-aware; on top of 1.9.7's `CLS x,y TO x2,y2` partial clear + `DRAWTEXT scale`, 1.9.6's AMOS-style multi-plane screen buffers (`SCREEN BUFFER/DRAW/SHOW/FREE/SWAP/COPY`, 8 slots, `DOUBLEBUFFER` folded in as shorthand), 1.9.5's bitmap-plane double-buffer, and the `FILEEXISTS` / `DOWNLOAD` / `IMAGE GRAB/SAVE` / compound-assignment (`+= -= *= /= ++ --`) work from 1.9.2–1.9.4. Graphics 1.0 milestone (1.9.0): full 2-D primitive set, blitter Phase 1, tilemap renderer, VSYNC pipeline, keyboard intrinsics, anti-alias toggle, two-word SPRITE/TILE/SHEET/IMAGE family. Next: **2.0** with sound. See `CHANGELOG.md` for the full versioned history and `docs/release-graphics-1.0.html` for the announcement page.
...
- **Mouse-over-sprite hit testing:** **`ISMOUSEOVERSPRITE(slot)`** returns **1** when the pointer is inside the slot's last-drawn rect (SCROLL-aware — sprite positions are world space, mouse coords are screen space, engine adjusts). **`SPRITEAT(x, y)`** returns the topmost visible slot at `(x, y)` or `-1`, with `Z` tie-breaking — ideal for "click to pick from a stack" use cases. Examples: `examples/ismouseoversprite_demo.bas`, `examples/gfx_sprite_at_demo.bas`. Pixel-perfect alpha sampling is planned (see `docs/mouse-over-sprite-plan.md`); until it ships, the BASIC helper below covers the bounding-box case if you don't want the engine path:
